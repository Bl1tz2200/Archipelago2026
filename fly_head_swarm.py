...
═══════════════════════════════════════════════════════════════════════════════
ЭТО КОПИЯ `fly_head.py`. Отличий от неё ровно четыре, все — маячок цепочки:

  1. `import socket`, Nodename и настройки BEACON_PORT / BEACON_ADDR / JOIN_WAIT_S;
  2. секция МАЯЧОК ЦЕПОЧКИ ниже: beacon(), joined(), wait_join();
...
`diff fly_head.py fly_head_swarm.py` — расхождений должно быть ровно четыре.
═══════════════════════════════════════════════════════════════════════════════
...
(п. 2.1.2). Выполнение назначенной фигуры (п. 2.1.3) в этот файл не входит.
...
# внутри команды любые беспроводные каналы. Обмен предельно простой, две строки:
#   лидер → хвостам:  "H <ID узла, к которому идём> <сколько яблок засчитано>"
...
# следующей, а если маячок молчит совсем — хвосты уходят на свой резервный отсчёт.
...
# поэтому лидер слышит собственные «H». Разбираются только «T» — доклады хвостов.
...
def beacon(target, apples_seen):
    """Сказать хвостам, к какому узлу идём и сколько яблок засчитано.

    Односторонне и без подтверждений: не дошло — хвост возьмёт следующую датаграмму
    через узел. Отказ сокета печатается и игнорируется: полёт лидера от маячка не
    зависит, а хвосты без него уйдут на свой резервный отсчёт по часам.
    """
    try:
        _BEACON.sendto(f"H {target} {apples_seen}".encode(), (BEACON_ADDR, BEACON_PORT))
    except OSError as exc:
        print(f"          маячок не ушёл: {exc}", flush=True)
...
def joined():
    ...
    while True:
        try:
            ...
            return
        parts = data.split()
        if len(parts) != 2 or parts[0] != b"T":
            continue                    # свой же «H» или чужая датаграмма на том же порту
        try:
            JOINED.add(int(parts[1]))
        except ValueError:
            pass
...
def watch(drone, base, fruit, found, deadline=None):
    ...
    for colour, x, y in fruit:
        ...
        # Узел зависания берём с метки под дроном: с неё же хвост отсчитает своё место.
        beacon(base[0], number)
        ...
        if fresh:
            ...
            wait_join(base[0], number, deadline)
        return True
    return False
        ...
        if start is None:
            ...
            for target in route:
                ...
                if time.time() > deadline:
                    ...
                    break
                # Голова цепочки перешла на новый узел следа — хвосты дописывают его
                # себе и подтягиваются. Шлём ДО перелёта: пока лидер летит, хвост уже
                # знает, куда двигаться, и цепочка идёт в ногу, а не рывками.
            ...
            for _ in range(GRID * 2):
                ...
                # След продолжается и на возврате: хвосты идут за головой домой тем же
                # порядком, а не остаются висеть там, где кончился поиск.
                beacon(path[0], len(found))
                try:
