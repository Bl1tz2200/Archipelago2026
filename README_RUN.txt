...
ROS 2 (source руками не нужен):
   ...
   ./run.sh flight --alt 0.5 --speed 0.3     ПРОСТОЙ ПОЛЁТ: арм → взлёт → змейка
                             по всему полю → посадка. Без «яблок», роя и бюджета —
                             этим проверяют, что дрон вообще летает по полю.
                             Маршрут без дрона: ./run.sh flight --dry-run
...
Если ./run.sh mission молчит и ничего не делает — это она же: get_telemetry() без
телеметрии FCU не возвращается никогда и не реагирует на Ctrl+C. У ./run.sh flight
на этот случай стоит сторож: он не зависнет, а через 6 секунд напишет, чего не
